 ...
 * sequence across chunks is harmless: persist_json_escape() only rewrites
 * bytes below 0x20, so multi-byte sequences pass through unchanged.
 */
static int write_escaped_field(int fd, const char *src)
{
    ...
    while (*src != '\0')
    {
        ...
        n = persist_json_escape(piece, out, sizeof(out));
