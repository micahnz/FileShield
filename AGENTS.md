...
- Protected-path globs: `*` (one segment) and `**` (zero or more segments) only; `?`, `[`, `]` and `\` are literal. A glob entry marks its wildcard-free base directory (a filesystem mark covers the rest of its filesystem, falling back to a mount mark where unsupported), and a malformed pattern is rejected at load time (fail closed)
