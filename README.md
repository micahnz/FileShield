...
- Every mutation (`remove`, `clear`, `prune`, all `session` commands) goes through the daemon's root-only control socket at `/run/fileshield.sock` (mode 0600, peer uid checked), so the daemon's in-memory lists and the state files change together and no reload is needed. Run the CLI as root.
...
- **Control socket**: the CLI's mutations travel over `/run/fileshield.sock` — root-owned, mode 0600, peer uid checked, and one bounded non-blocking read per connection (a slow client can never stall the daemon). `list`/`describe` read the state files directly; mutations fall back to direct file edits only when no listener answers (`ENOENT`/`ECONNREFUSED`), never on `EACCES` or another error. Session rules exist only in daemon memory, so every session command requires the running daemon.
