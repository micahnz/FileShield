...
- A glob entry marks its wildcard-free base directory (`~/.cloudflared`). A base that does not exist yet behaves like a missing exact path: it is skipped at startup and covered by the filesystem mark until it appears (reload to add the direct mark).
