...
- **Changed hash**: a `kdialog` prompt shows the rule pattern, the concrete binary, the old and new SHA-512 (16-hex prefixes; the full digests go to the journal), the target file and the command. **Update & Allow** replaces the pin and allows that access; **Deny**, window close and timeouts deny **this attempt only** and keep the old pin. The next access prompts again, subject to the dialog rate limiter.
