...
- A malformed pattern (relative, an unresolved `.` or `..` segment, `//` or a trailing slash in the wildcard part) is rejected at load time with a log message — it never silently protects nothing.
...
- A malformed pattern (relative, no static base, an unresolved `.` or `..` segment, or an empty segment such as `//` or a trailing slash after the wildcard part) is rejected at load with a log message — it never silently matches nothing.
